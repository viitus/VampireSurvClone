...
func _physics_process(delta):
	if target_array.size() > 0:
		...
		var distante_dif = global_position - player.global_position
		var return_speed = 100
		if abs(distante_dif.x) > 500 or abs(distante_dif.y) > 500:
			return_speed = 200
...
func _on_reset_pos_timer_timeout() -> void:
	var choose_diretion = randi() % 4
	reset_pos = player.global_position
	match choose_diretion:
		0:
			reset_pos.x += 50
		1:
			reset_pos.x -= 50
